...
```S
a:i32 // declearation
a; // declear a variable with its type infered by following usage.
...
{
    a:7 // new overriding declearation in the block scope
}

```
#### imutable, mutable and const
```S
const a:must_inicompile_time_const_expression
```
Const:
Declared using const keyword
Must be initialized to a constant expression, known at compile time
Implicitly immutable
Has a static lifetime
Immutable variable:
Declared using let keyword
Can be initialized to any expression, including runtime values
Explicitly immutable (let x is immutable, let mut x is mutable)
Has the same lifetime as the enclosing scope
