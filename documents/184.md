...
line comment */
/*
# we can use markdown in comment
`markCode(num:1,line:2,column:3)
*/
```
## new line

new line has its syntax meaning in S:

* after operand( var or func name is operand too) and before another operand : the same as add a ';'

* after operator and before operand: continue from a new line;

* before operator and after operand: depends on operator: special op like . means this., it's an operand, so it's add ;,  others is the same as continue from new line.
...
```S
TypeA
IA // interface A // not needed? just use type?
pubImutableVariable
pubMutableVariable_
_privateImutableVar
_privateMutableVar_
// Type member: field and function is the same as var, the mutability for function
// is about the instanace object.
SYMBOL_NAME // all upcase
AB // 
A_SYM // or append '_SYM' to make as snake case if needed
constant_name
a_con: 3i32 // append '_con' to make it as snake case if needed
```
```S
change:
fgh_ '_' read only 
...
## Primitive Types
primitive type start with lower case character
### Numeric types
```S
...
```
```S
// unit type: both value and type
nil// none; optional value type;absence of any optional value, it's the not selected value in option selection
void // return nothing; no value;absence of return value
// void is useful in polymophic type Task<A>; Task<void> the same as Task
void|nil // return nothing or return absent-value
never // never return; always abort; while(true){}
_: any value and any type when appear at type position
```
```S
T? // the same as
T|nil
...
`Num:Int|Unt|Flt
// polymophic <>
// polymophic type is fixed when it was used
`A<T>() {
    a: T
}
```
...
`b = i32(a)
`c = '3'
b = i32(c) // not like this
// it's the constructor syn
...
true false
```
>  polymophic Types is parameterized by other types
...
### char and string
```S
'c' // char or string literal , polymorphic
"\n"
'' // empty str
'cbc;'// literal str 
'ab''c' // ab'c
//literal str do not allow /n // take effect
...
"\u2605" // unicode ★
// str is utf8, can not be indexed directly, need to .bytes to get the under byte array

// ": interpolition and \x
// ': literal string
/”
 is template str that ' and “ are all usable
”/
...
```
> =can chained with any logical operator, for canditional check
## Variable
...
### expression block
a sequence of expressions, value returned from the block with `return` or retrurn the last expression value in the block if that one excutedj, add a `;` to the last expression to prevent the value return.
```S
{
4
6
...
// \ can only be after if expr that without else, or block
// add ; to last expr to prevent default yield
```
new design

with this design, the loop result can be returned as the block result when no ; appended
```S
// while:
k= i++<10 ? {
    b+=2;
...
k=i++<10？b++\\ // every b is yield

```

loop expression may return 3 type of objects, enumerable, value or both (tuple)

### enumeration expression
>~~not use this design: it requires space before afert %, we chose the design of implicit variable $, $i~~

> we use this design, $ is the this obj, $i is not good for embedded loop, and more charactors than i, space is not needed when no i,v
...
how to handle reverse enumration
### Implicit var
not used, not good for embedded enumeration
```S
IEnumrable % $i>7?$$
```
...
```S
// full syntax
`FuncA: (a:i32, b_:8i32)->i32 => a+b
// ->: return
// =>: assign logic
```
#### Idea
```S
// type fuction is Up case, common function is low case
// type function has implecit '.' op inside.
...
//when used with &, allocate B on heap and return a reference
// when used without &, allocated on stack and return instance.
b = &B();
// 
```

this design: can not identify anomymouse funcion (lambda), ''(){}"  is common and valide as two parts.
```S
func(a:i32, b:i32)i32{a + b}
func(a:i32, b:i32){a + b}
funcB(a:i32,b:str)i32 {a<6?return 3+5 8+8}
funcC(a:i32,b:str)(c:i32,d:f32) {c = 3 d = 5;} // add ';' to last expression prevent return
func((a:i32,b:f32))i32 //// any obj that has a,b of types
sndOfTriple ((_, y, _)){y} // any type info is not needed
sndOfTriple<T> ((_, y:T, _)){y} // needed?
```
```S
// undetermined paramter length
func(a,b,...c)// c is a list indexable and has length property
```

> we need arrow function to identify a function, especially for anonamous function: ()=>{}
```S
// to define a var that is a function
`c_ = 3
`a = (){c_++;}
// this is can be interpreted into 
`a = ()
{
    c_++;
}
// but if we ask every expression ended with ';'
// this problem would be solved
`a = ()// because if it's a obj, we should add ;
{
    c++;
}
```
...
`a = ()->c++;
```
and how to declair the function type with (){}
```S
a: ()int
// hard to parse
// so we choose
a: ->int
```
when declear a named function, we can remove ->?
initial implementation we always do -> to identify function.

we allow global function(type constructor) but no global var
```S
funcA(){

}
```
> so the must have signature: 
```S
func: (a:i32,b:i32)->i32 = a+b
// suggars
...
## Error Handling of Function

the error happens because the programer just want to do happy pass programming, we need a way to not allow just doing happy pass

i.e. the null reference error: the program does not handle the return the null value, which is not the happy pass, and treat the null as the same way of a valid object handling. to saving: explicitly mark a new type if it can be null: Object? in c#. In S, we do similar:

```S
// int? is the same as int | None
```

### function returned result
```S
// may return 3 kinds of result:
// always success
T 
// success, but None is a result that stand for negitive result, i.e. not found anything when searching
...
```
```Si
() // void, that is the function return nothing, different with none, none is the returned value that stands for nothing.
Result<(), MyError> // means function return nothing or return error

res:Result<Option<User>, DBErr>
//res # {
//  Ok(Some(user)) ? 'means a user was found successfully',
//  Ok(None) ? 'means no matching user was found',
//  Err(e)? 'means there was a database error.'
//}

res # {
  user:User? 'means a user was found successfully',
  None ? 'means no matching user was found',
  e:DBErr? 'means there was a database error.'
}
...
//func()?! // forward Error and panic on none
// handle negative result
//func()??{ $n #{ None?{}, Error?{}} } 

// error return: valid value become expression result but error is returned as function result
expression??
// error throw: forward valid value (incude T or None) but throw on Err
expression?!

// handle all
func()#{ v:T?{}, None?{}, err:Error?{} }  // #{}: overlook unhandled cases
// throw on unhandled, and handle case in pattern match clause
func()#!{ None?{}, v:T?{} }

func()#?{ None?T()}?! // None handdled as empty T and T forwarded, and Err is throw
```
...
type A(a:i32,b=6.1f32,c="song",~d:i32)
...
// d is not a member, just a parameter,use ~
```
> design decision: use : instead of the =
```S
// type F = 3|5|6.2
tp F: 3|5|6.2
// or
`F: 3|5|6.2 
// because : is the ofType syntaxor, we define new type from existing type
```
```S
ty A;
...
### general Type form
```S
`TypeName<Type1,Type2...;in1,in2...;outVar1,outVar2...>: TypePattern(field1:TypePattern, ...)
// in1,in2... is the dependent type parameter, 
// Dependent types are types parameterized by values rather than just types.

//type A<T;b;d>: (a@`c, c@>d, b:T){}
`A<T;3;o:i32>: (a:`c,b:T,e:>3, d:o){}
//A<i32;3;`bb>:(c:`cc) = v
// left side of '=' is the type pattern to match
A<i32;3;`bb>:(c:`cc) = v
`Int : 
```
## Interface
```S
// if is interface, we have ? as 'if'
inf IA() 
{

}
A:IA { // when A ofType IA
...
### any type
```S
_ // any type or value discarding
a: [_]//list of any type
```
### member visibility
```S
// private member (func and field) start with _
tp A(_a:int, b:5i32, __c:int) {}
// c is only used as paramerter
...
```
```S
// just store data for member match by position,
// we can remove field's name in type constructor
A(i32, i8)
a#(`a, `b) ? do-something
```
...
### Extension Methods
```S
 // for extension logic we can only define function, no member. we can override getter and setter and member function
// to share the extension method to other modules, just imp the file
// \n is meaningful, { at the same line of CT, it's extension logic of CT otherwise it's a pure scope.
A{ // ext to A, no override (which override to use, when imp several exts of A?)
        pubMethod(){}
        _priMethod{}
        pubMut_: B { // ext to B
            ->$ // override can happen in local ext
            v->$=v;
        }
}
```
...
### Property
#### new Design
> property is an instance of anonymous type which has a private filed and a set or get method
```S
// full syntax
`prop = (`Type1:()->Type1 => {
    `get: ()->Type1 => expr;
...
// 
`prop1 = Prop<Type1>(){.get=>expr;.set=>expr};
```
> get is used when use the property, set is used when asign value with '=' operator

> we can define 'get' and 'set' function on any type to implement value converter

can not use ={}, it is also assign a value from block
':{}'  is a dynamic type???

old design
need to find a meaningful operator: .{}
```S
A(){
    b_: CT.{ 
        // if we can define extension logic for CT, we can use $ here instead of $_
        ->$_ // underlie mutable
...
```S
@() // attach data, @(getter,setter)
@{} // attach logic
@[] // attach atribute, decorator
`A {
    b_: CT@( 
        // if we can define extension logic for CT, we can use $ here together with $_
        ->$_, // underlie mutable
        v->{$_=v;} // we use $_ instead of _$
        // $_ only meaningful when used in setter
...
## property validator and Changed (additinal feature in std lib)

> to demo s lang is supper dialectable, with the support of attribute 

implementation append logic to excute the function dictionary that can be added and removed,
only the property has setter and also has @[trigger] on the setter
...
// we do before set, after set in the property body
```
```S 
// debinding
a<!=b // break the b to a bind
a<!>b // break the 2 way bind
```
...
// but if we can store info about name and offet of member, as type meta data, we can support dynamic access
// we choose c[stringExpression] instead of c.(stringExpression)
...
### mutability
```S
// mut is shadow mut in S:
// content mutablity of reference is not counted
// for deep mutabality, we use readonly keyword
// so for readonly array reference and list, we do:
a: &int32[3] // imutable reference to array on heap, but content is mutable
b: &ReadonlyArray<int32;;3> = [2,3,5,4]
```
...
```
### Enum Union
Enum Union is the combination of Enum and Union, and it's the sum type of the subTypes.

_other names:_
* Tagged Union: a tag filed to discriminate the sub types.
* descriminated union: can be just any one type of the sub types
* sum type: the value range is sum the posibile values of the types
* variant type and enumeration type



```S
EnumUnion: type TypeName = SubType ('|'SubType)*[{memberLogic}] 
...
// if Type is public, Both Type and SubType are exported.
// SubType is also a field of the Type
// we can access subType:
SubType; Type.SubType

```
#### enum union implement
...
### unit type: one value type
```S
a: 3i32
// a is also init as 3i32
// a is imutable by nature, so no a_

`A = 3i32
// any var of A is const 
// enum
`A = B:3i32 | C:8i32 
// one value type can also be used as value
a = B// is possible, but should nt, because type is compile time concept
```

```S
// named void-unit type
`Red
`Color : Red | Blue | Yellow
// Blue and Yellow is new unit type, if it is in the union context and no type of the name found in current context.
// desugur
...
Enum is named value
```S
// maybe not needed. if we can customerize the key of enum union, and get the key value.
enum Color {
    Red, Blue, Green
}
enum Color { // inside the body, as subtype
    Red: 0xff0000, // use : to denotate ofType, Red is subtype 
    Green:0x00ff00
}
```
...
// todo: need to design a way to get attribute value
a := Key.????(Red, Color) or Color.Key(Red)???
// new design
//this is more better, with const type, it has all capbility of pure enum,
// but it intruduce additional tag filed compaired with pure enum
// the enum union way, we can assign same value to keys
`A = B:3i32 | C:8i32 || D:8i32
```

### Union
we can use slice or pointer converion to do similar thing
#### bool type
```S
// unit const type
tp true: 1// here we use tp because true is small case
tp false: 0
tp bool: true|false
```
...
#### merged type
```S
`A = B+C // A created from B and C, with all the member of B and C
...
## Summery of Types
we have predefined primitive types, by combining(producting) we create struct, tupple; by aggregation, we create Array, List, Set, and Map; by summing, we create enum union.
> Note: the type with members defined is called 'product type' (in contrast with sum type), because the value possibility is the multiplication of the value posibility of filed memebers.

> Cartesian set mathematic: the sum type and product type come from the Cartesian Set theory

> primitive type, product type, sum type, unit type
### Funciton Member
```S
...
A.funcA()->4
```
## anonymous type and pure-data Tuple
```S
(1,2)
(1,) 
// instead of use (1)
...
// (1,) + (2,3)
// a: (1,) as Typ
() is the void unit type
// any named type can be viewed as named Tuple

```
## Collection types: Array, List, Map and Set
> (Set | List {index}|Map):Collection(count):IEnumrable

### Collection Literal
```S
ICollection(count): IEnumerable
[x,x,x,x] // x can be IEnumberable
[...1..10] // from Range:IEnumberable
[1, ...4..8]
[1,3,5,6]
[(1,3),(2,3),(4,5)]
```
### Array
old design: not good for pattern match: T[3,4].
features: fixed length items of same type stored in the concatunated mem; 
```S
`Array<T;len:i32>:()
// Array is a dependent type that customerized by the len value parameter
a: i32[] // type
a: i32[3] // value
a:= [1,2,3]i32[]
a:i32[3] = [1,2,3]
...
```S
a:[i32;2,3]// 2 dementhinal array of i32
a:[i32;] // i32 array, demention determined at init time
// do we need jagged array?
jaggedA:[&[i32;];3] // jagged array
jaggedA
```

> A jagged array is an array whose elements are arrays, possibly of different sizes. A jagged array is sometimes called an "array of arrays." Its elements are reference types  and are initialized to null. 
### List
...
// enumrable
1..9 // 9 is not included
1..=9// 9 is included
'a'..='z'
9..1 // 1 is not included
-10..10
10..-10
1..2..9
10..2..-10
1.. // all natural number

..=-1 // all negitive number
```


###  index range
can count from back with the '^' operator
```S
`a = [0..=10]
a[1..=4] // 1,2,3,4
a[1..4] // 1,2,3
...
```
this design not compitable with value
```S
// [valueType; keyType]
a: [f32;string]
b: [f32;i32] =[(1,3.2), (2,3.3)]
// different with array, array [ValueType; number]
```
```S
a: [f32:string]
b: [f32|flt:i32] =[1:3.2, 2:3.3]
```
## Module
in file 
```S
...
when a object is allocated on heap, the owner is the heap, the heap will add an additional filed to trace the reference count, this filed is part of the heap management data, not belong to type.
```S
{
// `&` to allocate obj on heap: & is get reference operator. obj may on stack, when pair with Type is allocate on heap
//
a := &Str('abc') 
...
//b is outof scope, the counter is 1 now
log.info("$a")
//a is not used any more, the counter is 0 and the obj is freed from heap and it's free() will be called,
// of just at the end of the scope inject logic to decrease counter of heap-reference var and do the release.
}
```
...
```
## Type Matching
> value can be used as type, but except unit type, type can not be used as value
...
// problem: we can not differentiate | in pattern and the case operator.
// should we? ',': continue, semantic of ',' : find previous pattern match and use the value for new pattern match
// the problem: this 'free' style, give more freedom, there maybe other statement inserted between pattern arms.
  v  #pattern   ?if expression arm
    ,#pattern   ?{if code block expression arm}
    ,#pattern   ?if expression arm
```  
```S
// we choose this design: #{(pt?exp,)+}
// overlook unmatched case of value, or add a '_', to process all unmatched
  v #{
    pattern && condition|| condition   ?if_expression_arm,
    pattern   ?{if_code_block_expression_arm},
    pattern   ?if_expression_arm,
    _ ? any_arm
  }
```

```S
// partial match and passthrough all unmatched values
  v #? {
    pattern   ?if_expression_arm,
    pattern   ?{if_code_block_expression_arm},
    pattern   ?if_expression_arm,
...
```
```S
// partial match and thow when there is any unmatched value
  v #! {
    pattern   ?if_expression_arm,
    pattern   ?{if_code_block_expression_arm},
    pattern   ?if_expression_arm,
...
```js
  var btStyle = closeButton.style;
  btStyle.position = "absolute";
  btStyle.top = "5px";
  btStyle.right = "5px";
  btStyle.cursor = "pointer";
  btStyle.fontWeight = "bold";
```
```S

// create a temp context object, 
// usful for initilizing or config
closeButton.style.{
    positon = 'absolute'
    top = '5px'
    right = '5px'
