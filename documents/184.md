...
the declearion file generated contains info that identify the coro dll
...
type Array<T>(init:T = default(T), cap: i32, len: i32, data: *T)
c: i32[](init: 3, cap:5, len:3)
d: i32[3] // fix cap array explicitly
// compile detect, the array's capacity in the function is fixed, it will allocate it on stack
// 
