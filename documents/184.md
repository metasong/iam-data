...
```S
   ...
   u8 → u16 → u32 → u64 ⬎
      ↘     ↘     ↘      ptr
   i8 → i16 → int → i64 ⬏
// type is the encoding rule of value and layout of memebers
// a polymofic value is value with its i 
```


### Literal of Primitive Types
```S
// type epidecorator for integer
128 //the same as 128int
128i32 1.2f64
...
// : is ofType operator, used to constrain the obj, 
// = is valueDePattern operator
...
bb:=8i32 // new var with same binding of value
// pattern match explaining
variable pattern, then type pattern
