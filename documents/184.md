...
### Bit Operators
```S
the c bit operators are replaced: https://en.wikipedia.org/wiki/Bitwise_operations_in_C#Bitwise_operators
&	bitwise AND
...
```S
if we just use = for declear and mut, we can not declear same var in nested scope. becaue it is considered as mut. in other worlds, we need to defrencite init and mut
