...
```S
// type is just one type of pattern
// : is ofPattern operator, used to further contrain the pattern, 
// = is dePattern operator
a:1i32 // a is of the pattern of value 1i32, means it can only be 1i32, it's type is 1i32
aa:i32 = 1 // 1 do patter match into var aa, it is ofPattern i32, to make it success, 1 is i32
// pattern match explaining
variable pattern, then type pattern
...
// start with big case
B: 4 // mutable
// end with `_`?, because Type starts with big case
```
...
A(readOnly: i32, mutable_: Str, _private: i32, _privateMutable_: [i32], a=5i32) {
 _privateMemb: i32
 publicMemb: f32
 func(){
...
```S
int: 3
// the value memeber pattern, desuggar into:
int(value:3)
``` 
##### List pattern design
> Set | List {index}:Collection(count):IEnumrable

   [a,b,i(4..^4):c,w,..:f,i(6):d,e]
  fix every position
   any object that has
   i(): index pattern
...
##### logical pattern
> !: not, &: and; |:or
> : is also the 'and', but with low precedence
precedence: : < | < &
```S
