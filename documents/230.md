...
* order:  from the top most app template, go over every element in the template and then go down inside the element-template one by one from the first line to last line in the template, if 
