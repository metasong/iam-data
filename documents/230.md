...
1. change detection runs on **element's component**
    ...
    1. ngDoCheck: to dectect additional changes that may happens and can not detected by Angular change dectection (not from input value changes, any others?). i.e. in async pipe from the observerable 
        ...
        > 1. ngSwitchCase: ngOnChange could only caught change of the case expression reneed to eveluate experience on ngSwitch too. we need to compare the result from 2 expressions. in _matchCase: after all matches tested but still no match, use the default case.
