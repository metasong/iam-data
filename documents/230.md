...
1. the directive's input property set one by one -> 
1. ngOnChanges that happens after all input properties are set ->
...
1. ngDoCheck: to dectect additional changes that may happens and can not detected by Angular change dectection (not from input value changes, any others?).
