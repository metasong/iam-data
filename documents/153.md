...
* `none([ .. ])` is like `all([ .. ])`, but fulfillments and rejections are transposed. All Promises need to be rejected -- rejections become the fulfillment values and vice versa. **(fulfill: all reject, reject: any fulfill)**
* `any([ .. ])` is like `all([ .. ])`, but it ignores any rejections, so only one needs to fulfill instead of all of them. **(fulfill: any fulfill)**
...
* `allSettled([ .. ])`  resolves after all of the given promises have either fulfilled or rejected
