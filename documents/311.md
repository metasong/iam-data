...
// in tranducer only the last-executed step(xform) uses the 'a' to procuce result, others just pass 'a' into next execute-step, so xform definites the output of transducer
