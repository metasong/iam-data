...
//tf:transform
const map = tf => step => // step: reducer is previous-step transducer result, in this example it's the reault of map transducer
// 'a':result from last executed step, 
// here the filter is first executed step, although it's the next step of map in the `compose` function
// executed-steps are the reversing of the definited composed-steps 
  (a, c) => step(a, tf(c)); // step: in this example, it's arrayConcat
// filter(p) return a transducer
const filter = predicate => step => // step: the reducer from map transducer
