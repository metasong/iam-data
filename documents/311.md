...
//tf:transform
const map = tf => step => // step: in this example, it's arrayConcat
// 'a':result from last executed step, 
  (a, c) => step(a, tf(c)); 
// filter(p) return a transducer
const filter = predicate => step => //// step: reducer is previous-step transducer's result, in this example it's the reducer from map transducer
