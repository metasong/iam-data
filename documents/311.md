...
// so it can be used to take transducers(t:r=>r) and then pass in reducer as initial value.
...
const map = f => step => // step is previous step
  (a, c) => step(a, f(c)); // in this example, it's arrayConct
